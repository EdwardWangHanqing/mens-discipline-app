...
|---|---|---:|---|---|
| R-01 | Family Controls Distribution entitlement or provisioning is unavailable for the main app or a required extension | P0 | Phase 03–08 | Apple has assigned Family Controls (Distribution) to the developer account for the main-app path. Still validate distribution provisioning/archive/TestFlight and request separately for every extension actually introduced; no extension exists yet. Preserve the entitlement explanation. |
...
| R-27 | A stale, corrupt, or mismatched opaque FamilyActivitySelection leaves the diagnostic shield inconsistent or difficult to remove | P0 | Phase 03 selection/shield proof | Persist Apple's Codable selection locally without exposing token contents; report absent/empty/corrupt states; require usable authorization and non-empty explicit selection before Apply; isolate settings in one named store; keep Remove available and clear only that store. Real-device save/edit, restored-development-relaunch persistence, shield/remove, repeated apply/remove, and safe empty-selection behavior passed; deliberately corrupt state remains untested. |
