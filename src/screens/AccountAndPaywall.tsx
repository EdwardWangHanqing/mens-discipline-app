...
export function PaywallScreen({ onPurchase, onRestore, onOpenLegal, onOpenDeveloperControls }: {
  onPurchase: (plan: SubscriptionPlan) => Promise<AuthResult>;
  onRestore: () => Promise<AuthResult>;
  ...
  return (
    <Screen scroll testID="subscription-paywall">
      <View style={styles.paywallHeader}>
        ...
        <Eyebrow accent>VAEL MEMBERSHIP</Eyebrow>
        <Title>Your daily standard starts here.</Title>
        <Body muted>Daily men&apos;s performance training, guided structure, accountability, and momentum.</Body>
      </View>
      ...
      <View style={styles.plans}>
        <PlanCard selected={selected === 'annual'} title="Annual" price="$39.99 / year" badge="BEST VALUE" support="3 DAYS FREE · $3.33/month equivalent" onPress={() => setSelected('annual')} />
        <PlanCard selected={selected === 'monthly'} title="Monthly" price="$9.99 / month" badge="NO FREE TRIAL" support="Charged immediately through the App Store" onPress={() => setSelected('monthly')} />
      </View>
      ...
      <View style={styles.paywallActions}>
        ...
        <Text style={styles.legalCopy}>Payment is charged to your Apple ID. Subscriptions renew automatically unless cancelled at least 24 hours before the current period ends.</Text>
        {onOpenDeveloperControls ? <TextButton label="Developer Controls" onPress={onOpenDeveloperControls} color={colors.tertiary} /> : null}
      </View>
    </Screen>
  );
  ...
  async function purchase() {
    ...
    try { const result = await onPurchase(selected); if (!result.ok) setError(result.error); }
  ...
  async function restore() {
    ...
    try { const result = await onRestore(); if (!result.ok) setError(result.error); }
